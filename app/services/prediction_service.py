...
class PredictionService:
    ...
    def predict_batch(self, symbols: list[str]) -> BatchPredictResponse:
        ...
        for symbol in symbols:
            try:
                ...
                # Salva cada predição no banco de dados
                if self.db:
                    self._save_prediction_metric(symbol, last_date, price)
    ...
    def predict_sequence(self, symbol: str, days_ahead: int) -> SequencePredictResponse:
        ...
        # Salva a predição inicial (primeiro dia)
        if self.db and preds:
            try:
                today = datetime.date.today()
                price = preds[0]["predicted_close"]
                self._save_prediction_metric(symbol, today, price)
            except Exception as e:
                # Não falha a predição se houver erro ao salvar métrica
                print(f"Aviso: Não foi possível salvar métrica: {e}")
